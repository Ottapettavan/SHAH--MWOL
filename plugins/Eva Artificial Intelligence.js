...
Asena.addCommand({on: 'text', fromMe: wk, dontAddCommandList: true, deleteCommand: false}, (async (message, match) => {
    if (message.message.startsWith('bot') && conf.FULLEVA !== 'true') {        
        ...
        await axios.get('http://api.brainshop.ai/get?bid=159572&key=7rwTmhxDa07l3Hrpz&uid=' + unique_ident + '&msg=' + uren).then(async (response) => {
            var fins = ''                           
            if (conf.LANG !== 'EN') {
    ...
    try {
        ...
        convertToWav(file)
            .on('end', async () => {
                ...
                await axios.get('http://api.brainshop.ai/get?bid=159572&key=7rwTmhxDa07l3Hrpz&uid=' + unique_ident + '&msg=' + son).then(async (response) => {
                    var trmsg = ''
